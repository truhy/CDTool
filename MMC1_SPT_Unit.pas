...
Constructor T_MMC1_SPT.Create(Var In_SPC_SPT : T_SPC_SPT);
Begin
     SPC_SPT:=In_SPC_SPT;
     MMC1:=T_MMC1.Create;
End;

Destructor T_MMC1_SPT.Destroy;
Begin
     MMC1.Destroy;
End;

Function T_MMC1_SPT.Get_MMC1 : T_MMC1;
Begin
     Result:=MMC1;
End;

Function T_MMC1_SPT.Get_err_msg : PChar;
{ ************************************
...
Procedure T_MMC1_SPT.Do_read_rec_cap_CDB10;
{ ********************************************************
  Sends a read recorded capacity (CDB10) command to target
  device.

  Note: on some CDROM/CD writer drives they will return
  FFFFh and status OK.  It should return a  status error!!
  ******************************************************** }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, 8, 10);
     MMC1.Fill_CDB10_do_read_rec_cap(T_readCD_rec_cap(SPC_SPT.SPT.Get_p_Cdb^));

     SPC_SPT.V_Send_SCSI_cmd;
End;

Procedure T_MMC1_SPT.Do_read_subch_CDB10(In_MSF     : Byte;
                                         In_SubQ    : Byte;
                                         In_Params  : Byte;
                                         In_TrackNo : Byte);
{ **********************************************************
...
Begin
     ...
     Begin
          ...
          End;
     End;
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, Buf_len, 10);
     MMC1.Fill_CDB10_do_read_subch(T_read_subch(SPC_SPT.SPT.Get_p_Cdb^),
                                   In_MSF,
                                   In_SubQ,
                                   In_Params,
                                   In_TrackNo);
...
Procedure T_MMC1_SPT.Do_read_T_P_A_CDB10(In_MSF       : Byte;
                                         In_format    : Byte;
                                         In_trkSessNo : Byte);
{ ***********************************************************
...
Begin
     ...
     End;
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, Buf_len, 10);
     MMC1.Fill_CDB10_do_read_T_P_A(T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^),
                                   In_MSF,
                                   In_format,
                                   In_trkSessNo);
...
Procedure T_MMC1_SPT.Do_read_header_CDB10(In_MSF        : Byte;
...
Begin
     ...
     End;
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, Buf_len, 10);
     MMC1.Fill_CDB10_do_read_header(T_read_header(SPC_SPT.SPT.Get_p_Cdb^),
                                    In_MSF,
                                    In_MMCLBA_MSF);
...
Procedure T_MMC1_SPT.Do_read_header_MMCLBA_CDB10(In_MMCLBA : LongInt);
{ *************************************************************************
...
Procedure T_MMC1_SPT.Do_read_header_MSF_CDB10(In_MSF : T_MSF);
{ **********************************************************************
...
Procedure T_MMC1_SPT.Do_readCD_MSF_byFormat_CDB12(Var In_StartMSF        : T_MSF;
                                                  Var In_EndMSF          : T_MSF;
                                                      In_SectTypeFilter  : Byte;
                                                      In_ReadFormatFlags : Byte;
                                                      In_SubChSel        : Byte);
...
Begin
     ...
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, Buf_len, 12);
     MMC1.Fill_CDB12_do_readCD_MSF_byFormat(T_readCD_MSF(SPC_SPT.SPT.Get_p_Cdb^),
                                            In_StartMSF,
                                            In_EndMSF,
                                            In_SectTypeFilter,
                                            In_ReadFormatFlags,
...
Procedure T_MMC1_SPT.Do_set_CD_speed_CDB12(In_ReadSpeed  : Word;
                                           In_WriteSpeed : Word);
{ *************************************
  Sends a set CD speed (CDB12) command.
  ************************************* }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_OUT, 0, 12);
     MMC1.Fill_CDB12_do_set_CD_speed(T_set_CD_speed(SPC_SPT.SPT.Get_p_Cdb^),
                                     In_ReadSpeed,
                                     In_WriteSpeed);
...
Procedure T_MMC1_SPT.Do_readCD_byFormat_CDB12(In_Start_MMCLBA    : LongInt;
                                              In_N_sectors       : LongWord;
                                              In_SectTypeFilter  : Byte;
                                              In_ReadFormatFlags : Byte;
                                              In_SubChSel        : Byte);
...
Begin
     ...
     If In_N_sectors=0 Then
         N_sectors:=SPC_SPT.SPT.Get_data_buf_size Div Buf_len
     Else
         N_sectors:=In_N_sectors;
     Buf_len:=Buf_len*N_sectors;
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, Buf_len, 12);
     MMC1.Fill_CDB12_do_readCD_byFormat(T_readCD_CDB12(SPC_SPT.SPT.Get_p_Cdb^),
                                        In_Start_MMCLBA,
                                        N_sectors,
                                        In_SectTypeFilter,
                                        In_ReadFormatFlags,
...
Procedure T_MMC1_SPT.Do_read_buf_cap_CDB10;
{ ************************************************************
  Sends a read buffer capacity CDB10 command to target device.
  ************************************************************ }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, SizeOf(T_read_buf_cap_CDB10_block), 10);
     MMC1.Fill_CDB10_do_read_buf_cap(T_read_buf_cap_CDB10(SPC_SPT.SPT.Get_p_Cdb^));

     SPC_SPT.V_Send_SCSI_cmd;
End;

Procedure T_MMC1_SPT.Do_read_disc_info_CDB10;
{ ******************************************************
  Sends a read disc info CDB10 command to target device.
  Excludes returning of OPC table entries.
  ****************************************************** }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, SizeOf(T_read_disc_info_CDB10_block), 10);
     //Fills CDB10 for beginning fixed size part of block, i.e. excludes the length for OPC table entries.
     MMC1.Fill_CDB10_do_read_disc_info(T_read_disc_info_CDB10(SPC_SPT.SPT.Get_p_Cdb^), SizeOf(T_read_disc_info_CDB10_block));

     SPC_SPT.V_Send_SCSI_cmd;
End;

Procedure T_MMC1_SPT.Do_read_disc_info_with_OPC_CDB10;
{ ******************************************************
  Sends a read disc info CDB10 command to target device.
...
Begin
     ...
     If T_out_read_disc_info_CDB10_block(SPC_SPT.Get_data_buf^).No_of_OPC_tab_entries>0 Then
     Begin
          OPC_table_entries_total_bytes:=T_out_read_disc_info_CDB10_block(SPC_SPT.Get_data_buf^).No_of_OPC_tab_entries*8;
          SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, SizeOf(T_read_disc_info_CDB10_block)+OPC_table_entries_total_bytes, 10);
          MMC1.Fill_CDB10_do_read_disc_info(T_read_disc_info_CDB10(SPC_SPT.SPT.Get_p_Cdb^), SizeOf(T_read_disc_info_CDB10_block)+OPC_table_entries_total_bytes);

          SPC_SPT.V_Send_SCSI_cmd;
     End;
End;

Procedure T_MMC1_SPT.Do_read_track_info_CDB10(In_TrkBIT       : Byte;
                                              In_MMCLBA_TrkNo : LongInt);
{ *******************************************************
  Sends a read track info CDB10 command to target device.
  ******************************************************* }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_IN, SizeOf(T_read_track_info_CDB10_block), 10);
     MMC1.Fill_CDB10_do_read_track_info(T_read_track_info_CDB10(SPC_SPT.SPT.Get_p_Cdb^),
                                        In_TrkBIT,
                                        In_MMCLBA_TrkNo);
...
Procedure T_MMC1_SPT.Do_reserve_track_CDB10(In_Reverv_size : LongWord);
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_UNSPECIFIED, 0, 10);
     MMC1.Fill_CDB10_do_reserve_track(T_reserve_track_CDB10(SPC_SPT.SPT.Get_p_Cdb^),
                                      In_Reverv_size);

     SPC_SPT.V_Send_SCSI_cmd;
End;

Procedure T_MMC1_SPT.Do_synchronize_cache_CDB10(In_MMCLBA   : LongInt;
                                                In_N_blocks : Word);
{ *********************************************************
...
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_UNSPECIFIED, 0, 10);
     MMC1.Fill_CDB10_do_synchronize_cache(T_synchronize_cache_CDB10(SPC_SPT.SPT.Get_p_Cdb^),
                                          In_MMCLBA,
                                          In_N_blocks);
...
Procedure T_MMC1_SPT.Do_write_CDB10(In_DPO_FUA_RELADR : Byte;
                                    In_MMCLBA         : LongInt;
                                    In_N_blocks       : Word;
                                    In_block_size     : Word);
{ *********************************************
  ...
  ********************************************* }
Begin
     SPC_SPT.SPT.Fill_spt(SCSI_IOCTL_DATA_OUT, In_N_blocks*In_block_size, 10);
     MMC1.Fill_CDB10_do_write(T_write_CDB10(SPC_SPT.SPT.Get_p_Cdb^),
                              In_DPO_FUA_RELADR,
                              In_MMCLBA,
                              In_N_blocks,
                              In_block_size);
...
Function T_MMC1_SPT.Get_modesense6_DBD_last_used : Byte;
{ *********************************************
  Reads the CDB to get the DBD value
  that was used.  This assumes the previous
  command sent was mode sense CDB6.
  ********************************************* }
Begin
     Result:=T_mode_sense_CDB6(SPC_SPT.SPT.Get_p_Cdb^).DBD;
End;

Function T_MMC1_SPT.Get_modesense10_DBD_last_used : Byte;
{ *********************************************
  Reads the CDB to get the DBD value
  that was used.  This assumes the previous
  ...
  ********************************************* }
Begin
     Result:=T_mode_sense_CDB10(SPC_SPT.SPT.Get_p_Cdb^).DBD;
End;

Function T_MMC1_SPT.Is_sense6_block_desc_returned(PageCode : Byte) : Boolean;
{ ***************************************************
...
Begin
     ...
     SPC_SPT.Mode_sense_CDB6(MODE_SENSE_DBD_BLK_DESC,
                             PageCode,
                             SizeOf(T_mode_param_hdr_6));
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          { Checks whether any block descriptor/s are returned. }
     ...
     Begin
          ...
          SPC_SPT.Mode_sense_CDB6(MODE_SENSE_DBD_NO_BLK_DESC,
                                  PageCode,
                                  SizeOf(T_mode_param_hdr_6));
...
Function T_MMC1_SPT.Is_sense10_block_desc_returned(PageCode : Byte) : Boolean;
{ *************************************************
...
Begin
     { Sends a mode sense CDB10 with request for returning of block descriptor/s
       ...
       descriptor/s are used by the device. }
     SPC_SPT.Mode_sense_CDB10(MODE_SENSE_DBD_BLK_DESC,
                              PageCode,
     ...
     Begin
          ...
          SPC_SPT.Mode_sense_CDB10(MODE_SENSE_DBD_NO_BLK_DESC,
                                   PageCode,
                                   SizeOf(T_mode_param_hdr_10));
...
Function T_MMC1_SPT.Is_sense6_block_desc_returned_from_data : Boolean;
{ ************************************************
  ...
  ************************************************ }
Begin
     With T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^) Do
     Begin
...
Function T_MMC1_SPT.Is_sense10_block_desc_returned_from_data : Boolean;
{ ************************************************
  ...
  ************************************************ }
Begin
     With T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^) Do
     Begin
          { Checks whether any block descriptor/s were returned.
            The ReverseBytesToWord need not be used, instead MSB and LSB
            can be checked to both equate 0. }
          If SPC_SPT.SPC.ReverseBytesToWord(Block_desc_len_MSB,
...
Function T_MMC1_SPT.Calc_modepage6_ptr : Pointer;
{ **************************************************
...
Var
   Block_desc_len : Byte;
Begin
     Block_desc_len:=T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Block_desc_len div 8 * 8;

     Calc_modepage6_ptr:=Ptr(Integer(SPC_SPT.SPT.Get_data_buf)+
                             SizeOf(T_mode_param_hdr_6)+
                             Block_desc_len);

End;

Function T_MMC1_SPT.Calc_modepage10_ptr : Pointer;
{ **************************************************
  Calculates the address where the mode page begins.
  A pointer with the address is returned.
...
Var
   Block_desc_len : Byte;
Begin
     if (T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Reserved1 and 1)=0 then
     begin
          Block_desc_len:=SPC_SPT.SPC.ReverseBytesToWord(T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_MSB,
                                                         T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_LSB)
                          div 8 * 8;
     end
     else
     begin
     ...
     Calc_modepage10_ptr:=Ptr(Integer(SPC_SPT.SPT.Get_data_buf)+
                              SizeOf(T_mode_param_hdr_10)+
                              Block_desc_len);
End;

Function T_MMC1_SPT.Calc_sense6_return_cap_from_param_len(PageCode : Byte) : Byte;
{ ****************************************************************************************
...
Begin
     ...
     Begin
          ...
          Begin
               ...
               SPC_SPT.Mode_sense_CDB6(dbd,
                                       PageCode,
                                       Return_cap);
               If SPC_SPT.SPT.dev_io_success Then
               Begin
                    With T_mode_param_6_hdr_noblk_2_data(SPC_SPT.SPT.Get_data_buf^) Do
                    Begin
          ...
          Begin
               ...
               Return_cap:=SizeOf(T_mode_param_hdr_6)+
                                  T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Block_desc_len+
                                  2;
               SPC_SPT.Mode_sense_CDB6(MODE_SENSE_DBD_BLK_DESC,
                                       PageCode,
                                       Return_cap);
               If SPC_SPT.SPT.dev_io_success Then
               Begin
                    P:=Ptr(Integer(SPC_SPT.SPT.Get_data_buf)+Return_cap-1);
...
Function T_MMC1_SPT.Calc_sense10_return_cap_from_param_len(PageCode : Byte) : Word;
{ ****************************************************************************************
  This function is for compatibility of being able to issue sense commands to various
  CDROM drives without Win ASPI32 or the drive waiting for ever.
...
Begin
     ...
     Begin
          ...
          Begin
               ...
               SPC_SPT.Mode_sense_CDB10(dbd,
                                        PageCode,
                                        Return_cap);
               If SPC_SPT.SPT.dev_io_success Then
               Begin
                    With T_mode_param_10_hdr_noblk_2_data(SPC_SPT.SPT.Get_data_buf^) Do
                    Begin
                         Return_cap:=Return_cap+Param_len;
                    End;
               End
          ...
          Begin
               ...
               Return_cap:=SizeOf(T_mode_param_hdr_10)+
                           SPC_SPT.SPC.ReverseBytesToWord(T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_MSB,
                                                          T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_LSB)+
                           2;
               SPC_SPT.Mode_sense_CDB10(MODE_SENSE_DBD_BLK_DESC,
                                        PageCode,
...
Function T_MMC1_SPT.Calc_sense6_return_cap_from_mode_data_len(PageCode : Byte) : Byte;
{ ****************************************************************************************
  This function is for compatibility of being able to issue sense commands to various
  CDROM drives without Win ASPI32 or the drive waiting for ever.
...
Function T_MMC1_SPT.Calc_sense10_return_cap_from_mode_data_len(PageCode : Byte) : Word;
{ ****************************************************************************************
  This function is for compatibility of being able to issue sense commands to various
  CDROM drives without Win ASPI32 or the drive waiting for ever.
...
Begin
     ...
     Begin
          ...
          Begin
               { Checks whether the mode data len is at least greater than the header. }
               If SPC_SPT.SPC.ReverseBytesToWord(Mode_data_len_MSB,
               ...
               Begin
                    ...
                    Return_cap:=SPC_SPT.SPC.ReverseBytesToWord(Mode_data_len_MSB,
                                                               Mode_data_len_LSB)+2;
               End
               Else
               Begin
...
Function T_MMC1_SPT.Calc_sense6_return_cap_from_mode_data_and_param_len(PageCode : Byte) : Byte;
{ **************************************************************************
...
Function T_MMC1_SPT.Calc_sense10_return_cap_from_mode_data_and_param_len(PageCode : Byte) : Word;
{ ***************************************************************************
...
Procedure T_MMC1_SPT.Do_select6_read_err_rec(In_err_rec_param    : Byte;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          Block_desc_len:=T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Block_desc_len
                          div 8 * 8;
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_6_read_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
          ...
          Else
          Begin
               P_read_err_rec_param_page:=SPC_SPT.SPT.Get_data_buf;
               ...
               MMC1.Fill_mode_pg_read_err_rec(P_read_err_rec_param_page,
                                              ...
                                              In_read_retry_count,
                                              True);
               Alloc_size:=T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Mode_data_len+1;
               //Alloc_size:=SizeOf(T_mode_param_6_read_err_rec_noblk_data)+Block_desc_len;
          End;
     End
     Else
     Begin
          MMC1.Fill_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf);
          With T_mode_param_6_read_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
     ...
     SPC_SPT.Mode_select_CDB6(Alloc_size, False);
End;

Procedure T_MMC1_SPT.Do_select10_read_err_rec(In_err_rec_param    : Byte;
                                              In_read_retry_count : Byte);
{ ********************************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_10_read_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
                    MMC1.Fill_mode_pg_read_err_rec(@Read_err_rec,
                                                   Read_err_rec.Param_len,
                                                   In_err_rec_param,
                                                   In_read_retry_count,
                                                   True);
                    Alloc_size:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Mode_data_len_MSB,
          ...
          Begin
               ...
               MMC1.Fill_mode_pg_read_err_rec(P_read_err_rec_param_page,
                                              ...
                                              In_read_retry_count,
                                              True);
               Alloc_size:=SPC_SPT.SPC.ReverseBytesToWord(T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Mode_data_len_MSB,
                                                          T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Mode_data_len_LSB)+2;
               //Alloc_size:=SizeOf(T_mode_param_10_read_err_rec_noblk_data)+Block_desc_len;
          End;
     End
     Else
     Begin
          MMC1.Fill_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf);
          With T_mode_param_10_read_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               //Fill_block_desc(@Block_desc);
               MMC1.Fill_mode_pg_read_err_rec(@Read_err_rec,
     ...
     SPC_SPT.Mode_select_CDB10(Alloc_size, False);
End;

Procedure T_MMC1_SPT.Do_sense6_read_err_rec;
{ ***********************************************************************
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          SPC_SPT.Mode_sense_CDB6(Get_modesense6_DBD_last_used,
                                  MMC_MODE_PAGE_READ_ERR_REC,
                                  Return_cap);
     End;
End;

Procedure T_MMC1_SPT.Do_sense10_read_err_rec;
{ ************************************************************************
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          SPC_SPT.Mode_sense_CDB10(Get_modesense10_DBD_last_used,
                                   MMC_MODE_PAGE_READ_ERR_REC,
                                   Return_cap);
     End;
End;

Procedure T_MMC1_SPT.Do_select6_write(Var In_write_params : T_pub_write_params);
{ *******************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_6_write_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
          ...
          Else
          Begin
               P_write_param_page:=SPC_SPT.SPT.Get_data_buf;
...
Procedure T_MMC1_SPT.Do_select10_write(Var In_write_params : T_pub_write_params);
{ *******************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_10_write_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
                    MMC1.Fill_mode_pg_write(@Write,
                                            Write.Param_len,
                                            In_write_params,
...
Procedure T_MMC1_SPT.Do_sense6_write;
{ ***********************************************************
...
Procedure T_MMC1_SPT.Do_sense10_write;
{ ************************************************************
...
Procedure T_MMC1_SPT.Do_select6_veri_err_rec(In_err_rec_param    : Byte;
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_6_veri_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
          ...
          Else
          Begin
               P_veri_err_rec_param_page:=SPC_SPT.SPT.Get_data_buf;
...
Procedure T_MMC1_SPT.Do_select10_veri_err_rec(In_err_rec_param    : Byte;
                                              In_veri_retry_count : Byte);
{ **********************************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_10_veri_err_rec_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
                    MMC1.Fill_mode_pg_veri_err_rec(@Veri_err_rec,
                                                   Veri_err_rec.Param_len,
                                                   In_err_rec_param,
...
Procedure T_MMC1_SPT.Do_sense6_veri_err_rec;
{ *************************************************************************
...
Procedure T_MMC1_SPT.Do_sense10_veri_err_rec;
{ **************************************************************************
...
Procedure T_MMC1_SPT.Do_select6_CD(In_inactivity_mul : Byte;
                                   In_sec_per_MSF    : Word;
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_6_CD_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
          ...
          Else
          Begin
               P_CD_param_page:=SPC_SPT.SPT.Get_data_buf;
...
Procedure T_MMC1_SPT.Do_select10_CD(In_inactivity_mul : Byte;
                                    In_sec_per_MSF    : Word;
                                    In_frames_per_MSF : Word);
{ ******************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_10_CD_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
                    MMC1.Fill_mode_pg_CD(@CD,
                                         CD.Param_len,
                                         In_inactivity_mul,
...
Procedure T_MMC1_SPT.Do_sense6_CD;
{ ********************************************************
...
Procedure T_MMC1_SPT.Do_sense10_CD;
{ *********************************************************
...
Procedure T_MMC1_SPT.Do_select6_CD_audio_ctrl(Var In_CD_audio_ctrl : T_pub_CD_audio_ctrl);
{ ******************************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_6_CD_audio_ctrl_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
          ...
          Else
          Begin
               P_CD_audio_ctrl_param_page:=SPC_SPT.SPT.Get_data_buf;
...
Procedure T_MMC1_SPT.Do_select10_CD_audio_ctrl(Var In_CD_audio_ctrl : T_pub_CD_audio_ctrl);
{ ****************************************************************
...
Begin
     ...
     Begin
          ...
          If Block_desc_len=0 Then
          Begin
               With T_mode_param_10_CD_audio_ctrl_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
               Begin
                    MMC1.Fill_mode_pg_CD_audio_ctrl(@CD_audio_ctrl,
                                                    CD_audio_ctrl.Param_len,
                                                    In_CD_audio_ctrl,
...
Procedure T_MMC1_SPT.Do_sense6_CD_audio_ctrl;
{ *******************************************************************
...
Procedure T_MMC1_SPT.Do_sense10_CD_audio_ctrl;
{ ********************************************************************
...
Procedure T_MMC1_SPT.Do_sense6_CD_cap_mech_st_sub6;
{ ******************************************************************************************
...
Begin
     ...
     If Not SPC_SPT.SPT.dev_io_success Then
     Begin
          { Retrieve the full CD capabilities and mechanism status parameter list. }
          SPC_SPT.Mode_sense_CDB6(MODE_SENSE_DBD_NO_BLK_DESC,
                                  MMC_MODE_PAGE_CD_CAP_MECH_ST,
...
Procedure T_MMC1_SPT.Do_sense10_CD_cap_mech_st_sub6;
{ ******************************************************************************************
...
Procedure T_MMC1_SPT.Do_sense6_CD_cap_mech_st;
{ ****************************************************************************************
...
Procedure T_MMC1_SPT.Do_sense10_CD_cap_mech_st;
{ *****************************************************************************************
  Retrieve CD capabilities and mechanism status parameters using mode sense CDB10 to target
  device.  The T_GenSCSI will make the call to WNASPI32.
...
Procedure T_MMC1_SPT.Do_read_rec_cap_out_CDB10(Var LastMMCLBA     : LongInt;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_readCD_rec_cap_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               LastMMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(MMCLBA_HiByte,
                                                             MMCLBA_HiMiByte,
                                                             MMCLBA_LoMiByte,
                                                             MMCLBA_LoByte);
               MMCLBABlockLen:=SPC_SPT.SPC.ReverseBytesToLongWord(Len_HiByte,
                                                                  Len_HiMiByte,
                                                                  Len_LoMiByte,
                                                                  Len_LoByte);
          End;
...
Procedure T_MMC1_SPT.Do_read_disc_info_out_CDB10(Var Out_read_disc_info_CDB10_block : T_out_read_disc_info_CDB10_block);
Begin
     Do_read_disc_info_CDB10;
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          Out_read_disc_info_CDB10_block.DataLen:=SPC_SPT.SPC.ReverseBytesToWord(
                                                  T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DataLen_HiByte,
                                                  T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DataLen_LoByte);
          Out_read_disc_info_CDB10_block.Erasable_StateOfLastSess_DiscStatus:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Erasable_StateOfLastSess_DiscStatus;
          Out_read_disc_info_CDB10_block.No_of_first_trk:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).No_of_first_trk;
          Out_read_disc_info_CDB10_block.No_of_sess:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).No_of_sess;
          Out_read_disc_info_CDB10_block.First_trk_no_in_last_sess:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).First_trk_no_in_last_sess;
          Out_read_disc_info_CDB10_block.Last_trk_no_in_last_sess:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_trk_no_in_last_sess;
          Out_read_disc_info_CDB10_block.DID_V_DBC_V_URU:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DID_V_DBC_V_URU;
          Out_read_disc_info_CDB10_block.DiscType:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscType;
          Out_read_disc_info_CDB10_block.Reserved1:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Reserved1;
          Out_read_disc_info_CDB10_block.Reserved2:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Reserved2;
          Out_read_disc_info_CDB10_block.Reserved3:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Reserved3;
          Out_read_disc_info_CDB10_block.DiscID:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                                 T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscID_HiByte,
                                                 T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscID_HiMiByte,
                                                 T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscID_LoMiByte,
                                                 T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscID_LoByte);
          Out_read_disc_info_CDB10_block.Last_sess_lead_in_start_MSF.Reserved:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_sess_lead_in_start_MSF_HiByte;
          Out_read_disc_info_CDB10_block.Last_sess_lead_in_start_MSF.M:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_sess_lead_in_start_MSF_HiMiByte;
          Out_read_disc_info_CDB10_block.Last_sess_lead_in_start_MSF.S:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_sess_lead_in_start_MSF_LoMiByte;
          Out_read_disc_info_CDB10_block.Last_sess_lead_in_start_MSF.F:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_sess_lead_in_start_MSF_LoByte;
          Out_read_disc_info_CDB10_block.Last_start_for_lead_out_MSF.Reserved:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_start_for_lead_out_MSF_HiByte;
          Out_read_disc_info_CDB10_block.Last_start_for_lead_out_MSF.M:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_start_for_lead_out_MSF_HiMiByte;
          Out_read_disc_info_CDB10_block.Last_start_for_lead_out_MSF.S:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_start_for_lead_out_MSF_LoMiByte;
          Out_read_disc_info_CDB10_block.Last_start_for_lead_out_MSF.F:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Last_start_for_lead_out_MSF_LoByte;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte0:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte7;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte1:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte6;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte2:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte5;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte3:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte4;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte4:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte3;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte5:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte2;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte6:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte1;
          Out_read_disc_info_CDB10_block.DiscBarCode_Byte7:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DiscBarCode_Byte0;
          Out_read_disc_info_CDB10_block.Reserved4:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Reserved4;
          Out_read_disc_info_CDB10_block.No_of_OPC_tab_entries:=T_read_disc_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).No_of_OPC_tab_entries;
     End;
End;

Procedure T_MMC1_SPT.Do_read_disc_info_with_OPC_out_CDB10(Var Out_read_disc_info_CDB10_block : T_out_read_disc_info_CDB10_block);
Begin
...
Procedure T_MMC1_SPT.Do_read_track_info_out_CDB10(    In_TrkBIT       : Byte;
                                                      In_MMCLBA_TrkNo : LongInt;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          Out_read_track_info_CDB10.DataLen:=SPC_SPT.SPC.ReverseBytesToWord(
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DataLen_HiByte,
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).DataLen_LoByte);
          Out_read_track_info_CDB10.TrkNo:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkNo;
          Out_read_track_info_CDB10.SessNo:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).SessNo;
          Out_read_track_info_CDB10.Reserved:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Reserved;
          Out_read_track_info_CDB10.Damage_Copy_TrkMode:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).Damage_Copy_TrkMode;
          Out_read_track_info_CDB10.RT_Blank_Packet_FP_DataMode:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).RT_Blank_Packet_FP_DataMode;
          Out_read_track_info_CDB10.NWA_V:=T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).NWA_V;
          Out_read_track_info_CDB10.TrkStart_MMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                                     T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkStart_MMCLBA_HiByte,
                                                     T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkStart_MMCLBA_HiMiByte,
                                                     T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkStart_MMCLBA_LoMiByte,
                                                     T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkStart_MMCLBA_LoByte);
          Out_read_track_info_CDB10.NextWr_MMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                                   T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).NextWr_MMCLBA_HiByte,
                                                   T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).NextWr_MMCLBA_HiMiByte,
                                                   T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).NextWr_MMCLBA_LoMiByte,
                                                   T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).NextWr_MMCLBA_LoByte);
          Out_read_track_info_CDB10.FreeBlocks:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                                T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FreeBlocks_HiByte,
                                                T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FreeBlocks_HiMiByte,
                                                T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FreeBlocks_LoMiByte,
                                                T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FreeBlocks_LoByte);
          Out_read_track_info_CDB10.FixPackSize:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                                 T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FixPackSize_HiByte,
                                                 T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FixPackSize_HiMiByte,
                                                 T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FixPackSize_LoMiByte,
                                                 T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).FixPackSize_LoByte);
          Out_read_track_info_CDB10.TrkSize:=SPC_SPT.SPC.ReverseBytesToLongInt(
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkSize_HiByte,
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkSize_HiMiByte,
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkSize_LoMiByte,
                                             T_read_track_info_CDB10_block(SPC_SPT.SPT.Get_data_buf^).TrkSize_LoByte);
     End;
End;

Procedure T_MMC1_SPT.Do_read_subch_aud_st_out_CDB10(Var Out_audioStatus : Byte);
{ *************************************************
...
Begin
     ...
     Out_audioStatus:=T_read_subch_data_hdr(SPC_SPT.SPT.Get_p_Cdb^).Audio_status;
End;

Procedure T_MMC1_SPT.Do_read_subch_curr_MMCLBA_CDB10(Var Out_abs_MMCLBA : LongInt;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_read_subch_CD_curr_pos_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               Out_abs_MMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(AbsAdr_HiByte,
                                                                 AbsAdr_HiMiByte,
                                                                 AbsAdr_LoMiByte,
                                                                 AbsAdr_LoByte);
               Out_rel_MMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(TrkRelAdr_HiByte,
                                                                 TrkRelAdr_HiMiByte,
                                                                 TrkRelAdr_LoMiByte,
                                                                 TrkRelAdr_LoByte);
          End;
...
Procedure T_MMC1_SPT.Do_read_subch_curr_MSF_CDB10(Var Out_abs_MSF : T_MSF;
...
Procedure T_MMC1_SPT.Do_read_subch_MCN_out_CDB10(Var Out_MC_valid : Boolean;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          If (T_read_subch_MCN_data(SPC_SPT.SPT.Get_data_buf^).MCN.Valid AND 128)<>0 Then
              Out_MC_valid:=True
          Else
              Out_MC_valid:=False;
          ...
          For i:=1 To 13 Do
          Begin
               Case T_read_subch_MCN_data(SPC_SPT.SPT.Get_data_buf^).MCN.N[i] Of
                    '0'..'9': ASCII_num:=T_read_subch_MCN_data(SPC_SPT.SPT.Get_data_buf^).MCN.N[i];
               Else
...
Procedure T_MMC1_SPT.Do_read_subch_ISRC_out_CDB10(    In_TrackNo   : Byte;
                                                  Var Out_TC_valid : Boolean;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success Then
     Begin
          If (T_read_subch_ISRC_data(SPC_SPT.SPT.Get_data_buf^).ISRC.Valid  AND 128)<>0 Then
              Out_TC_valid:=True
          Else
              Out_TC_valid:=False;
          ...
          Out_ISRC_str:='';
          With T_read_subch_ISRC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               For i:=1 To 12 Do
               Begin
...
Function T_MMC1_SPT.Do_read_T_P_A_TOC_MMCLBA_first_out_CDB10(Var Out_first_trkNo : Byte;
                                                             Var Out_last_trkNo  : Byte;
                                                             Var Out_TOC_desc    : T_out_read_T_P_A_TOC_desc_MMCLBA;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success then
     Begin
          With T_read_T_P_A_TOC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               TOC_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                            Hdr.Data_len_LoByte);
               ...
               Begin
                    ...
                    Out_TOC_desc.StartMMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(TOC_trk_desc.Adr_HiByte,
                                                                                TOC_trk_desc.Adr_HiMiByte,
                                                                                TOC_trk_desc.Adr_LoMiByte,
                                                                                TOC_trk_desc.Adr_LoByte);
                    There_is_TOC_data:=True;
                    SPC_SPT.SPT.Desc_data_offset:=SizeOf(T_read_T_P_A_TOC_data);
               End;
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_TOC_MMCLBA_next_out_CDB10(Var Out_TOC_desc : T_out_read_T_P_A_TOC_desc_MMCLBA) : Boolean;
Var TOC_data_len       : Word;
    There_is_TOC_data  : Boolean;
    P_desc_data_offset : Pointer;
Begin
     There_is_TOC_data:=False;
     If (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Cmd=MMC_CMD_READ_TOC_PMA_ATIP) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Format=MMC_READ_T_P_A_FORMAT_TOC) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).MSF=MMC_READ_T_P_A_MMCLBA) AND
        SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_read_T_P_A_TOC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               TOC_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                            Hdr.Data_len_LoByte);
          End;
          If (TOC_data_len+2)>=(SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_TOC_trk_desc_data)) Then
          Begin
               P_desc_data_offset:=SPC_SPT.SPT.Get_data_buf;
               Inc(LongWord(P_desc_data_offset), SPC_SPT.SPT.Desc_data_offset);

               With T_read_T_P_A_TOC_trk_desc_data(P_desc_data_offset^) Do
               Begin
                    Out_TOC_desc.ADR_Ctrl:=ADR_Control;
                    Out_TOC_desc.TrackNo:=TrackNo;
                    Out_TOC_desc.StartMMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(Adr_HiByte,
                                                                                Adr_HiMiByte,
                                                                                Adr_LoMiByte,
                                                                                Adr_LoByte);
               End;
               There_is_TOC_data:=True;
               SPC_SPT.SPT.Desc_data_offset:=SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_TOC_trk_desc_data);
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_TOC_MSF_first_out_CDB10(Var Out_first_trkNo : Byte;
                                                          Var Out_last_trkNo  : Byte;
                                                          Var Out_TOC_desc    : T_out_read_T_P_A_TOC_desc_MSF;
                                                              Track_no        : Byte) : Boolean;
Var TOC_data_len      : Word;
...
Function T_MMC1_SPT.Do_read_T_P_A_TOC_MSF_next_out_CDB10(Var Out_TOC_desc : T_out_read_T_P_A_TOC_desc_MSF) : Boolean;
Var TOC_data_len       : Word;
    There_is_TOC_data  : Boolean;
    P_desc_data_offset : Pointer;
Begin
     ...
     If (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Cmd=MMC_CMD_READ_TOC_PMA_ATIP) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Format=MMC_READ_T_P_A_FORMAT_TOC) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).MSF=MMC_READ_T_P_A_MSF) AND
        SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_read_T_P_A_TOC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
...
Function T_MMC1_SPT.Do_read_T_P_A_sess_MMCLBA_out_CDB10(Var Out_first_sessNo : Byte;
                                                        Var Out_last_sessNo  : Byte;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success then
     Begin
          With T_read_T_P_A_sess_info_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               If (SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
               ...
               Begin
                    ...
                    Begin
                         ...
                         Out_sess_desc.StartMMCLBA:=SPC_SPT.SPC.ReverseBytesToLongInt(Adr_HiByte,
                                                                                      Adr_HiMiByte,
                                                                                      Adr_LoMiByte,
                                                                                      Adr_LoByte);
                    End;
...
Function T_MMC1_SPT.Do_read_T_P_A_sess_MSF_out_CDB10(Var Out_first_sessNo : Byte;
                                                     Var Out_last_sessNo  : Byte;
...
Function T_MMC1_SPT.Do_read_T_P_A_full_TOC_first_out_CDB10(Var Out_first_sessNo  : Byte;
                                                           Var Out_last_sessNo   : Byte;
                                                           Var Out_full_TOC_desc : T_out_read_T_P_A_full_TOC_desc;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success then
     Begin
          With T_read_T_P_A_full_TOC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               Full_TOC_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                                 Hdr.Data_len_LoByte);
               ...
               Begin
                    ...
                    SPC_SPT.SPT.Desc_data_offset:=SizeOf(T_read_T_P_A_full_TOC_data);
               End;
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_full_TOC_next_out_CDB10(Var Out_full_TOC_desc : T_out_read_T_P_A_full_TOC_desc) : Boolean;
Var Full_TOC_data_len      : Word;
    There_is_full_TOC_data : Boolean;
    P_desc_data_offset     : Pointer;
Begin
     There_is_full_TOC_data:=False;
     If (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Cmd=MMC_CMD_READ_TOC_PMA_ATIP) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Format=MMC_READ_T_P_A_FORMAT_FULL_TOC) AND
        SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_read_T_P_A_full_TOC_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               Full_TOC_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                                 Hdr.Data_len_LoByte);
          End;
          If (Full_TOC_data_len+2)>=(SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_full_TOC_trk_desc_data)) Then
          Begin
               P_desc_data_offset:=SPC_SPT.SPT.Get_data_buf;
               Inc(LongWord(P_desc_data_offset), SPC_SPT.SPT.Desc_data_offset);
               ...
               End;
               There_is_full_TOC_data:=True;
               SPC_SPT.SPT.Desc_data_offset:=SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_full_TOC_trk_desc_data);
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_PMA_first_out_CDB10(Var Out_PMA_desc : T_out_read_T_P_A_PMA_desc) : Boolean;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success then
     Begin
          With T_read_T_P_A_PMA_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               PMA_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                            Hdr.Data_len_LoByte);
               ...
               Begin
                    ...
                    SPC_SPT.SPT.Desc_data_offset:=SizeOf(T_read_T_P_A_PMA_data);
               End;
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_PMA_next_out_CDB10(Var Out_PMA_desc : T_out_read_T_P_A_PMA_desc) : Boolean;
Var PMA_data_len       : Word;
    There_is_PMA_data  : Boolean;
    P_desc_data_offset : Pointer;
Begin
     There_is_PMA_data:=False;
     If (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Cmd=MMC_CMD_READ_TOC_PMA_ATIP) AND
        (T_read_T_P_A(SPC_SPT.SPT.Get_p_Cdb^).Format=MMC_READ_T_P_A_FORMAT_PMA) AND
        SPC_SPT.SPT.dev_io_success Then
     Begin
          With T_read_T_P_A_PMA_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               PMA_data_len:=SPC_SPT.SPC.ReverseBytesToWord(Hdr.Data_len_HiByte,
                                                            Hdr.Data_len_LoByte);
          End;
          If (PMA_data_len+2)>=(SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_PMA_desc_data)) Then
          Begin
               P_desc_data_offset:=SPC_SPT.SPT.Get_data_buf;
               Inc(LongWord(P_desc_data_offset), SPC_SPT.SPT.Desc_data_offset);
               ...
               End;
               There_is_PMA_data:=True;
               SPC_SPT.SPT.Desc_data_offset:=SPC_SPT.SPT.Desc_data_offset+SizeOf(T_read_T_P_A_PMA_desc_data);
          End;
     End;
...
Function T_MMC1_SPT.Do_read_T_P_A_ATIP_out_CDB10(Var Out_ATIP_desc : T_out_read_T_P_A_ATIP_desc) : Boolean;
...
Begin
     ...
     If SPC_SPT.SPT.dev_io_success then
     Begin
          With T_read_T_P_A_ATIP_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               //If (SPC_SPT.ReverseBytesToWord(Hdr.Data_len_HiByte,
...
Procedure T_MMC1_SPT.Do_sense6_CD_cap_mech_st_sub6_out(Var Out_CD_cap_mech_st : T_pub_CD_cap_mech_st);
{ ****************************************************************************************
  Retrieve CD capabilities and mechanism status parameters using mode sense CDB6 to target
...
Begin
     ...
     If T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Block_desc_len=0 Then
     Begin
          With T_mode_param_6_CD_cap_mech_st_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
     ...
     Else
     Begin
          P_CD_cap_mech_st:=Ptr(LongWord(SPC_SPT.SPT.Get_data_buf)+
                                SizeOf(T_mode_param_hdr_6)+
                                T_mode_param_hdr_6(SPC_SPT.SPT.Get_data_buf^).Block_desc_len);
     End;
...
Procedure T_MMC1_SPT.Do_sense10_CD_cap_mech_st_sub6_out(Var Out_CD_cap_mech_st : T_pub_CD_cap_mech_st);
{ *****************************************************************************************
  Retrieve CD capabilities and mechanism status parameters using mode sense CDB10 to target
  device.  The T_GenSCSI will make the call to WNASPI32.  Friendly version.
  ***************************************************************************************** }
...
Begin
     ...
     If SPC_SPT.SPC.ReverseBytesToWord(T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_MSB,
                                       T_mode_param_hdr_10(SPC_SPT.SPT.Get_data_buf^).Block_desc_len_LSB)=0 Then
     Begin
          With T_mode_param_10_CD_cap_mech_st_noblk_data(SPC_SPT.SPT.Get_data_buf^) Do
          Begin
               P_CD_cap_mech_st:=@CD_cap_mech_st;
          End;
     End
...
Procedure T_MMC1_SPT.Do_sense6_CD_cap_mech_st_out(Var Out_CD_cap_mech_st : T_pub_CD_cap_mech_st);
{ ****************************************************************************************
  Retrieve CD capabilities and mechanism status parameters using mode sense CDB6 to target
  device.  The T_GenSCSI will make the call to WNASPI32.  Friendly version.
  *************************************************************************************** }
...
Procedure T_MMC1_SPT.Do_sense10_CD_cap_mech_st_out(Var Out_CD_cap_mech_st : T_pub_CD_cap_mech_st);
{ *****************************************************************************************
  Retrieve CD capabilities and mechanism status parameters using mode sense CDB10 to target
  device.  The T_GenSCSI will make the call to WNASPI32.  Friendly version.
  ***************************************************************************************** }
